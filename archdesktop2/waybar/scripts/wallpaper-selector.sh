...
generate_thumbnails() {
    find "$WALLPAPER_DIR" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.gif" -o -iname "*.webp" -o -iname "*.webm" \) | while read -r img; do
        ...
        if [ ! -f "$thumbnail" ] || [ "$img" -nt "$thumbnail" ]; then
            ...
            magick "$img[0]" -resize 200x200^ -gravity center -extent 200x200 "$thumbnail" 2>/dev/null
        fi
    done
}
...
if [ -n "$selected" ]; then
    ...
    if [ -f "$wallpaper_path" ]; then
        ...
        if [ "${extension,,}" = "mp4" ]; then
            ...
            magick "$wallpaper_path" "$HOME/.cache/last_wallpaper_static.jpg"
            ...
            waypaper --fill fill --wallpaper "$wallpaper_path" &
